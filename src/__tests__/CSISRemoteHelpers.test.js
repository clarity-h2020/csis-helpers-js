...
beforeAll(async (done) => {
    ...
    if (process.env.ORIGIN) {
        ...
        _CSISRemoteHelpers.csisClient.defaults.headers.common['Origin'] = process.env.Origin;
    }
    ...
    if (cookie) {
        ...
        _CSISRemoteHelpers.csisClient.defaults.headers.common['Cookie'] = cookie;
    } else {
...
afterAll(() => {
    ...
    if ((process.env.CSIS_USERNAME && process.env.CSIS_PASSWORD) || (process.env.COOKIE && process.env.ORIGIN)) {
        ...
        delete _CSISRemoteHelpers.csisClient.defaults.headers.common['Cookie'];
        delete _CSISRemoteHelpers.csisClient.defaults.headers.common['Origin'];
    }
...
describe('Remote API tests with authentication', () => {
    ...
    it('[PROD] test get EMIKAT Credentials', async (done) => {
        const emikatCredentials = await _CSISRemoteHelpers.getEmikatCredentialsFromCsis();
        expect.assertions(2);
        expect(emikatCredentials).toBeDefined();
        expect(emikatCredentials).not.toBeNull();
        done();
    ...
    it('[PROD] test get datapackage resources for eu-gl:hazard-characterization', async (done) => {
        const resourcesApiResponse = await _CSISRemoteHelpers.getDatapackageResourcesFromCsis(undefined, "a8ff7930-4a9f-4289-8246-3383ba13c30f");
...
//export default CSISRemoteHelpersTests;